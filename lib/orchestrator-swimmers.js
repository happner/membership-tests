...
OrchestratorSwimmers.prototype.onChange = function($happn, name, update, callback) {
  ...
  if (update.state == 0) {
    $happn.log.warn('(change) %s added %s', name, subject);
    $happn.emit('cluster/log', {level: 'warn', args: ['(change) %s added %s', name, subject]});
  ...
  if (update.state == 2) {
    $happn.log.warn('(change) %s removed %s', name, subject);
    $happn.emit('cluster/log', {level: 'warn', args: ['(change) %s removed %s', name, subject]});
...
OrchestratorSwimmers.prototype.onUpdate = function($happn, name, update, callback) {
  var subject = update.meta.name;
  if (update.state == 0) {
    $happn.log.warn('(update) %s added %s', name, subject);
    $happn.emit('cluster/log', {level: 'warn', args: ['(update) %s added %s', name, subject]});
    $happn.emit('swimmer/arrives', {
      at: name,
      subject: subject
    });
  ...
  if (update.state == 2) {
    $happn.log.warn('(update) %s removed %s', name, subject);
    $happn.emit('cluster/log', {level: 'warn', args: ['(update) %s removed %s', name, subject]});
    $happn.emit('swimmer/departs', {
      at: name,
      subject: subject
    });
